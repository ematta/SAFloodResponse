...
abstract class BaseRepository {
    ...
    protected suspend fun <T> safeFirestoreCall(operation: suspend () -> T): Result<T> {
        return try {
    ...
    protected suspend fun <T> safeNetworkCall(operation: suspend () -> T): Result<T> {
        return try {
            val result = operation()
            Result.success(result)
        } catch (e: FirebaseException) {
    ...
    protected fun mapFirebaseError(exception: Throwable): String {
        return FirebaseErrorMapper.getErrorMessage(exception)
    }

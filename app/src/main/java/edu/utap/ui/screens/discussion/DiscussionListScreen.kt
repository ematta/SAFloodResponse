...
fun DiscussionListScreen(
    ...
    floodReportViewModel: FloodReportViewModel,
    ...
    // Fetch flood reports to determine thread color coding
    val floodReports by floodReportViewModel.allReports.collectAsState()

    LaunchedEffect(Unit) {
        viewModel.fetchAllThreads()
        // Ensure flood reports loaded
        floodReportViewModel.fetchAllReports()
    }
            ...
            if (isLoading) {
                ...
                LazyColumn(
                    ...
                    items(allThreads) { thread ->
                        // Determine flood status by matching thread to its flood report
                        val status = floodReports.find { it.reportId == thread.reportId }?.status
                        val floodStatus = status?.lowercase()
                        ThreadCard(
                            thread = thread,
 ...
 * @param floodStatus The flood status to display as a colored indicator.
 */
@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun ThreadCard(
    ...
    onClick: () -> Unit,
    floodStatus: String? = null // status string from flood report
) {
    Card(
        onClick = onClick,
        modifier = Modifier
        ...
        Row(
            ...
            if (floodStatus != null) {
                // Red for active, yellow for pending, green for all other flood statuses
                val color = when (floodStatus.lowercase()) {
                    "active" -> Color.Red
...
fun ThreadCardPreview(
    ...
    ThreadCard(thread = thread, onClick = {}, floodStatus = "active")
}

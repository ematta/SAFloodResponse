...
open class AuthViewModel(
    ...
    override fun register(
        ...
        viewModelScope.launch {
            if (!networkUtils.isNetworkAvailable(context)) {
                val msg = "No network connection"
                stateManager.updateState(AuthState.Error.Network(msg))
                function(false, msg)
                return@launch
            }
            val registerResult = authRepository.registerUser(email, password, name, role)
            if (registerResult.isSuccess) {
                val firebaseUser = registerResult.getOrNull()!!
                val syncResult = authRepository.syncUser(firebaseUser)
                if (syncResult.isSuccess) {
                    val firestoreUser = syncResult.getOrNull()!!
                    stateManager.updateCurrentUser(firestoreUser)
                    cacheUser(firestoreUser)
                    stateManager.updateState(AuthState.Idle.Authenticated, firestoreUser)
                    function(true, null)
                } else {
                    val err = syncResult.exceptionOrNull()?.message ?: "User sync failed"
                    stateManager.updateState(AuthState.Error.Authentication(err))
                    function(false, err)
                }
            } else {
                val err = registerResult.exceptionOrNull()?.message ?: "Registration failed"
                stateManager.updateState(AuthState.Error.Authentication(err))
                function(false, err)
            }
        }
        ...
        viewModelScope.launch {
            ...
            if (authResult.isSuccess) {
                ...
                if (syncResult.isSuccess) {
                    ...
                    val errorMsg = syncResult.exceptionOrNull()?.message ?: "User sync failed"
                    stateManager.updateState(AuthState.Error.Authentication(errorMsg))
                    function(false, errorMsg)
                }
            } else {
     ...
     */
    override fun hasPermission(requiredRole: String): Boolean {

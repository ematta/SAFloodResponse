...
class FirebaseStorageUtilTest {
    ...
    fun setup() {
        ...
        // Stub retry time configuration on FirebaseStorage mock
        io.mockk.justRun { mockStorage.setMaxUploadRetryTimeMillis(any()) }
        io.mockk.justRun { mockStorage.setMaxDownloadRetryTimeMillis(any()) }
    ...
    fun `uploadProfileImage should return failure when getting download URL fails`() = runTest {
        ...
        every { mockDownloadUrlTask.exception } throws testException
        coEvery { mockDownloadUrlTask.await() } returns mockUri
